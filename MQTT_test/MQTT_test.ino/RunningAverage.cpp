...
{
    _size = n;
    _ar = (byte*) malloc(_size * sizeof(byte));
    if (_ar == NULL) _size = 0;
    clr();
    lastAvg;
}
...
void RunningAverage::addValue(byte f)
{
    if (_ar == NULL) return;
    _sum -= _ar[_idx];
    _ar[_idx] = f;
...
byte RunningAverage::getAverage()
{
...
byte RunningAverage::getElement(uint8_t idx)
{
...
void RunningAverage::fillValue(byte value, int number)
{
    clr();
