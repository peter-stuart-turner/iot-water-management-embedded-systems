...
char MQTT_GC::_sendAT(char *command, unsigned long waitms)
{
  ...
  //  SoftSerial.println(PrevMillis);
  //  SoftSerial.println(currentMillis);
...
void MQTT_GC::serialEvent(void)
{
   //SoftSerial.println("debug_serialEvent");
  while (Serial.available())
  {
    ...
    if (MQTT.TCP_Flag == false)
    {
      ...
      if (inChar == '\n')
      {
        ...
        stringComplete = true;
        SoftSerial.print(MQTT.inputString);
        ...
        else if (strstr(MQTT.inputString, "CONNECT") != NULL)
        {
          GSM_Response = 4;
          MQTT.TCP_Flag = true;
          SoftSerial.println("MQTT.TCP_Flag = True");
          MQTT.AutoConnect();
          MQTT.pingFlag = true;
    ...
    else
    {
      ...
      if ((ReceivedMessageType >= CONNECT) && (ReceivedMessageType <= DISCONNECT))
      {
        ...
        while ( (NextLengthByte == true) && (MQTT.TCP_Flag == true))
        {
          if (Serial.available())
          {
            ...
            if ((((Cchar & 0xFF) == 'C') && ((inChar & 0xFF) == 'L') && (MQTT.length == 0)) || (((Cchar & 0xFF) == '+') && ((inChar & 0xFF) == 'P') && (MQTT.length == 0)))
            {
              ...
              MQTT.MQTT_Flag = false;
              MQTT.pingFlag = false;
              SoftSerial.println("Disconnecting");
            }
            else
            {
              if ((inChar & 128) == 128)
        ...
        SoftSerial.println(MQTT.length);
        ...
        if (MQTT.TCP_Flag == true)
        {
          ...
          //SoftSerial.write(ReceivedMessageType);
          ...
          else if (ReceivedMessageType == PUBLISH)
          {
            ...
            {
              SoftSerial.print(MQTT.inputString[iter]);
          ...
          {
            SoftSerial.print("Message ID :");
            MQTT.publishREL(0, MQTT.inputString[0] * 256 + MQTT.inputString[1]);
            SoftSerial.println(MQTT.inputString[0] * 256 + MQTT.inputString[1]) ;
          ...
          else if (ReceivedMessageType == PINGREQ)
          {
            MQTT.TCP_Flag = false;
            MQTT.pingFlag = false;
            SoftSerial.println("Disconnecting1");
      ...
      {
        SoftSerial.print("Received :Unknown Message Type :");
        SoftSerial.println(inChar);
      }
    }
  }
}
